    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-500 dark:text-slate-400">
          {isComplete
            ? 'Vue comparative des performances des algorithmes entraînés'
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {modelResults.map((result, idx) => (
              <div
                key={idx}
                className={`bg-white dark:bg-slate-800 rounded-xl p-6 border-2 shadow-sm transition-all ${winner.modelName === result.modelName
                  ? 'border-green-400 bg-green-50 dark:bg-green-900/10 dark:border-green-500/50'
                  : 'border-gray-200 dark:border-slate-700'
                  }`}
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white">{result.modelName}</h3>
                    ...
                    Object.entries(result.metrics).map(([key, value], mIdx) => {
                      ...
                      return (
                        <div key={mIdx} className="flex items-center justify-between">
                          <span className="text-sm text-gray-600 dark:text-slate-400 capitalize">{key}</span>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600 dark:text-slate-400">Temps d'entraînement</span>
                    <span className="font-bold text-gray-900 dark:text-white">{(typeof result.trainingTime === 'number' ? result.trainingTime.toFixed(2) : '0.00')}s</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          {/* Comparison Chart */}
          <div className="bg-white dark:bg-slate-800 rounded-xl p-8 border border-gray-200 dark:border-slate-700 shadow-sm mb-8">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Comparaison des métriques</h3>
