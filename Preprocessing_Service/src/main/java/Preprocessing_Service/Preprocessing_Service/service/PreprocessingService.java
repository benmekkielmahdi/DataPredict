...
public class PreprocessingService {
    ...
    public Dataset importDataset(MultipartFile file, String description, Long userId) {
        try {
            ...
            // Send Notification
            if (userId != null) {
                logger.info("Sending IMPORT_COMPLETE notification to userId: {} for datasetId: {}", userId,
                        savedDataset.getId());
                firebaseNotificationService.sendNotificationToUser(
                        userId,
                        "Dataset Imported",
                        "Dataset '" + file.getOriginalFilename() + "' has been successfully imported.",
                        Map.of("datasetId", String.valueOf(savedDataset.getId()), "type", "IMPORT_COMPLETE"));
            } else {
                logger.warn("UserId is null, skipping IMPORT_COMPLETE notification for datasetId: {}",
                        savedDataset.getId());
            }
    ...
    public Dataset applyPreprocessing(Long datasetId, String configJson, Long userId)
        ...
        // Send Notification
        // Prioritize the dataset owner's ID, fallback to the passed userId (which might
        // be default 1) only if dataset owner is null
        Long targetUserId = dataset.getUserId();
        if (targetUserId == null) {
            targetUserId = userId;
        }

        if (targetUserId != null) {
            logger.info("Sending EXPORT_COMPLETE notification to userId: {} (Dataset Owner) for datasetId: {}",
                    targetUserId,
                    savedDataset.getId());
            firebaseNotificationService.sendNotificationToUser(
                    targetUserId,
                    "Dataset Exported",
                    "Dataset '" + originalName + "' has been successfully processed and exported.",
                    Map.of("datasetId", String.valueOf(savedDataset.getId()), "type", "EXPORT_COMPLETE"));
        } else {
            logger.warn("UserId is null, skipping EXPORT_COMPLETE notification for datasetId: {}",
                    savedDataset.getId());
        }
