...
public class TrainingService {
    ...
    public TrainingResponse processTraining(TrainingRequest request, Long userId) {
        ...
        for (AlgorithmRecommendation algo : topAlgorithms) {
            try {
                ...
                // Save to database
                saveTrainingRecord(request.getDatasetId(), result, request.getDescription(), userId,
                        request.getTaskType());
        ...
        TrainingResponse response = buildResponse(results);

        // 4. Send Firebase notification to user
        try {
            if (userId != null && !results.isEmpty()) {
                TrainingResult bestResult = results.stream()
                ...
                Double accuracy = getDoubleMetric(bestResult.getMetrics(), "accuracy");
                String datasetName = request.getDatasetId().contains("/")
                        ? request.getDatasetId().substring(request.getDatasetId().lastIndexOf("/") + 1)
                        : request.getDatasetId();

                firebaseNotificationService.sendTrainingCompleteNotification(
                        userId,
                        bestResult.getAlgorithmName(),
                        accuracy,
                        datasetName);
                log.info("Training complete notification sent to user_{}", userId);
            }
        } catch (Exception e) {
            log.error("Failed to send training notification to user_{}: {}", userId, e.getMessage());
        }
    ...
    public List<TrainingRecord> getHistory(Long userId) {
        return trainingRecordRepository.findByUserIdOrderByDateDesc(userId);
    }

    public Map<String, Object> getDashboardStats(Long userId) {
        Map<String, Object> stats = new HashMap<>();
        stats.put("totalDatasets", trainingRecordRepository.countByUserId(userId));
        stats.put("modelsTrained", trainingRecordRepository.countByStatusAndUserId("success", userId));

        Double bestAcc = trainingRecordRepository.findBestAccuracy(userId);
        stats.put("bestAccuracy", bestAcc != null ? Math.round(bestAcc * 1000) / 10.0 : 0.0);

        stats.put("systemStatus", "healthy");
        stats.put("lastUpdated", LocalDateTime.now());

        List<TrainingRecord> history = trainingRecordRepository.findByUserIdOrderByDateDesc(userId);
    ...
    private void saveTrainingRecord(String datasetId, TrainingResult result, String description, Long userId,
            String taskType) {
        try {
            ...
            // Build the record with appropriate metrics
            TrainingRecord.TrainingRecordBuilder builder = TrainingRecord.builder()
                    .datasetName(
            ...
            // Set classification metrics or 0.0 for regression
            if (isClassification) {
            ...
            // Set regression metrics or 0.0 for classification
            ...
            TrainingRecord record = builder.build();
            trainingRecordRepository.save(record);
            log.info("Saved training record for {} ({})", result.getAlgorithmName(), taskType);
        } catch (Exception e) {
