...
public class TrainingController {
    ...
    public ResponseEntity<TrainingResponse> trainModels(@RequestBody TrainingRequest request,
        ...
        TrainingResponse response = trainingService.processTraining(request, userId);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/history")
    public ResponseEntity<?> getHistory(@RequestHeader(value = "X-User-Id", defaultValue = "1") Long userId) {
        return ResponseEntity.ok(trainingService.getHistory(userId));
    }

    @GetMapping("/stats")
    public ResponseEntity<?> getStats(@RequestHeader(value = "X-User-Id", defaultValue = "1") Long userId) {
        return ResponseEntity.ok(trainingService.getDashboardStats(userId));
    }
